...
############################################################################################################
# EEG CORTICAL SURFACE ELECTRODE ATLAS (MAIN ANALYSIS) #
############################################################################################################
############################################################################################################
...
SGIGR1_aperiodicactivity_electrodeatlas$depth <- factor(sub("\\..*$", "", row.names(SGIGR1_aperiodicactivity_electrodeatlas)), levels = c("deep", "superficial"), ordered = T) 
...
# Brodmann cortical areas list
electrodes <- c("Brodmann.45", "Brodmann.9", "Brodmann.8", "Brodmann.4.6")

#Depth-specific (superficial/deep) R1 and fooof measures for final study sample in Brodmann atlas
R1_aperiodicactivity_electrodeatlas_brodmann <- readRDS("/Volumes/Hera/Projects/corticalmyelin_development/EEG/R1_aperiodicactivity_brodmann.RDS") #generated by /image_processing/EEG_aperiodic/fooof_aperiodicactivity.R
...
R1.aperiodic.maineffect.gams(input.depth.df = R1_aperiodicactivity_electrodeatlas_brodmann$superficial, aperiodic.measure = "exponent", output.df.name = "R1exponent_superficial_maineffect_brodmann.RDS")
R1.aperiodic.maineffect.gams(input.depth.df = R1_aperiodicactivity_electrodeatlas_brodmann$deep, aperiodic.measure = "exponent", output.df.name = "R1exponent_deep_maineffect_brodmann.RDS")
...
R1.aperiodic.interaction.gams(input.depth.df = SGIGR1_aperiodicactivity_electrodeatlas_brodmann, aperiodic.measure = "exponent", output.df.name = "R1exponent_depth_interaction_brodmann.RDS")

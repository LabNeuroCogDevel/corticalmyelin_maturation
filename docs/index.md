...
### Quantification ogithubf Aperiodic Activity
...
This project quantifies across-region correlations between brain maps, including whole-brain correlations between R1 and myelin-related measures (MBP, MWF) and frontal lobe-only correlations between development maps and indexes of cortical hierarchy (S-A axis, cytoarchitectural variation). To determine the statistical significance of these spatial correlations, autocorrelation preserving null models were constructed using [BrainSmash](https://brainsmash.readthedocs.io/en/latest/index.html). The procedure to compute correlation p-values with BrainSmash nulls requires 1. a parcellated distance matrix for your cortical atlas of interest, and 2. parcellated brain maps (empirical) to use for creating spatial autocorrelation-preserving surrogate maps (nulls).
...
Surrogate maps (2) were created for whole-brain R1 [/results/brainsmash_nulls/create_R1_surrogatemaps.py](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/brainsmash_nulls/create_R1_surrogatemaps.py), the frontal lobe S-A axis [ /results/brainsmash_nulls/create_SAaxis_surrogatemaps.py](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/brainsmash_nulls/create_SAaxis_surrogatemaps.py), and the frontal lobe cytoarchitectural variation gradient [/results/brainsmash_nulls/create_bigbrain_surrogatemaps.py](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/brainsmash_nulls/create_bigbrain_surrogatemaps.py).
...
* Myelin basic protein gene expression was obtained from vertex-level gene expression data generated from the AHBA by [Wagstyl et al., 2023, elife](https://elifesciences.org/reviewed-preprints/86933v1). The script [/results/R1_corticalmyelin_anatomy/myelin_maps/create_AHBAmagicc_genemap.sh](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/R1_corticalmyelin_anatomy/myelin_maps/create_AHBAmagicc_genemap.sh) gets MBP gene maps by interfacing with [/results/R1_corticalmyelin_anatomy/myelin_maps/create_AHBAmagicc_genemap.py](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/R1_corticalmyelin_anatomy/myelin_maps/create_AHBAmagicc_genemap.py) to pull gene expression and gene info  csvs and produce a parcellated MBP cifti.
* Myelin water fraction imaging was quantified in HCP-MMP regions by projecting the MNI-space myelin imaging template from [Liu et al., 2019, Journal of Neuroimaging](https://onlinelibrary.wiley.com/doi/10.1111/jon.12657) to the cortical surface and parcellating it to produce a MWF cifti. This was done in [/results/R1_corticalmyelin_anatomy/myelin_maps/parcellate_MWFmap.sh](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/R1_corticalmyelin_anatomy/myelin_maps/parcellate_MWFmap.sh), which calls [/results/R1_corticalmyelin_anatomy/myelin_maps/parcellate_MWFmap.py](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/R1_corticalmyelin_anatomy/myelin_maps/parcellate_MWFmap.py).
* Sensorimotor-association (S-A) axis ranks for HCP-MMP regions were obtained from the [archetypal S-A axis repo](https://github.com/PennLINC/S-A_ArchetypalAxis) 
...
### Charting Deep-to-superficial and Hierarchical Axes of Myelin Maturation 
...
### BrainSmash Significance
The significance of correlations between developmental maps and hierarchically-expressed variability in cortical properties and cytoarchitecture was assessed using autocorrelation-preserving S-A axis surrogates and cytoarchitectural variation surrogates (detailed above). Visualization of example surrogates and significance testing was executed in [/results/brainsmash_nulls/R1_depthdevelopment_brainsmash.Rmd](https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/brainsmash_nulls/R1_depthdevelopment_brainsmash.Rmd), which [can be viewed here](https://htmlpreview.github.io/?https://github.com/LabNeuroCogDevel/corticalmyelin_maturation/blob/main/results/brainsmash_nulls/R1_depthdevelopment_brainsmash.html).
